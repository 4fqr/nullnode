...
class Terminal {
        ...
        return {
            '~': {
                ...
                contents: {
                    ...
                    'practice_notes.txt': { type: 'file', content: 'Practice notes:\n- Explore ls, cat, pwd, history\n- Try python -c "print(\'Hello\')"\n- Experiment but stay safe — only simulated outputs here.' },
                    'readme_for_binary.txt': { type: 'file', content: 'Binary inspection notes:\n- Look for readable strings\n- Check imports and function names\n- Use a sandbox for dynamic analysis' },
                    'ad_summary.txt': { type: 'file', content: 'AD Summary (simulated):\n- Domain: EXAMPLE.LOCAL\n- Controllers: dc01\n- Admin groups: Domain Admins\n- Potential misconfigs: unconstrained delegation' },
                    'cloud_exposed_config.txt': { type: 'file', content: 'Cloud config (simulated):\n- public_bucket: true\n- iam_role: admin\n- env_secret: NOT_SET (should be set in secrets manager)' },
                    'example.py': { type: 'file', content: 'print("Hello from example.py")\n# Example script for demonstration' },
                    'script.py': { type: 'file', content: '# placeholder script\n# Use this file to practice python commands' },
                    'notes.txt': { type: 'file', content: 'Remember to practice and to use safe, legal targets for testing.' }
                }
            }
        };
    }
    ...
    executeCommand(commandLine) {
        // Render command prompt safely: prompt is controlled by app, commandLine is user input
        // Do not insert user content as HTML to avoid XSS.
        // Write prompt (HTML allowed) and command as text
        const promptNode = document.createElement('div');
        const promptSpan = document.createElement('span');
        promptSpan.className = 'terminal-prompt';
        promptSpan.textContent = `user@nullsector:${this.currentPath}$`;
        promptNode.appendChild(promptSpan);
        const cmdSpan = document.createElement('span');
        cmdSpan.className = 'terminal-user-input';
        cmdSpan.textContent = ' ' + commandLine;
        promptNode.appendChild(cmdSpan);
        this.output.appendChild(promptNode);
        ...
        switch (command.toLowerCase()) {
            ...
            default:
                recognized = false;
                // Command name comes from user input — render safely with textContent
                const notFound = document.createElement('div');
                const notFoundSpan = document.createElement('span');
                notFoundSpan.className = 'terminal-error';
                notFoundSpan.textContent = `Command not found: ${command}`;
                notFound.appendChild(notFoundSpan);
                this.output.appendChild(notFound);
                this.output.scrollTop = this.output.scrollHeight;
                this.writeLine('Type "help" to see available commands.');
        }

        // Emit a custom event so external pages can react (e.g. learning page exercise tracking)
        // Events include a `recognized` boolean to indicate the terminal recognized the command name
        try {
            const evt = new CustomEvent('terminal:command', { detail: { command, args, raw: commandLine, recognized } });
            if (this.container) this.container.dispatchEvent(evt);
            // also dispatch on document for broader listening
    ...
    showHelp() {
        ...
        this.writeLine('<span class="terminal-success">Available Commands:</span>', true);
        commands.forEach(([cmd, desc]) => {
            this.writeLine(`  <span class="terminal-command">${cmd.padEnd(20)}</span> ${desc}`, true);
        });
    }
    ...
    ls(args) {
        ...
        this.writeLine(colors.join('  '), true);
    }
    ...
    cat(filename) {
        if (!filename) {
            this.writeLine('<span class="terminal-error">cat: missing file operand</span>', true);
            return;
        }
        if (filename === 'welcome.txt') {
            ...
            // filename is user-provided — render as text within an error-styled element
            const nf = document.createElement('div');
            const nfSpan = document.createElement('span');
            nfSpan.className = 'terminal-error';
            nfSpan.textContent = `cat: ${filename}: No such file or directory`;
            nf.appendChild(nfSpan);
            this.output.appendChild(nf);
            this.output.scrollTop = this.output.scrollHeight;
        }
    }
    ...
    python(args) {
        if (args.length === 0 || args[0] === '--version') {
            ...
            if (code.includes('print')) {
                ...
                this.writeLine('<span class="terminal-success">Code executed successfully</span>', true);
            }
        } else {
    ...
    gcc(args) {
        if (args.length === 0) {
            ...
            // args[0] is user input — render as text inside a success element
            const ok = document.createElement('div');
            const okSpan = document.createElement('span');
            okSpan.className = 'terminal-success';
            okSpan.textContent = `Compilation successful: ${args[0]} -> a.out`;
            ok.appendChild(okSpan);
            this.output.appendChild(ok);
            this.output.scrollTop = this.output.scrollHeight;
        }
    }
    ...
    // writeLine: append a new line to terminal output.
    // By default, 'text' is treated as plain text (safe). If the caller needs to inject
    // HTML produced by the application itself, pass allowHtml = true.
    writeLine(text, allowHtml = false) {
        const line = document.createElement('div');
        if (allowHtml) {
            // trusted HTML coming from application code (not user input)
